    ...
    this.createNodes = (buffer, sanitizer) => {
      ...
      for (let iterator = 0; iterator < nodeBufferLength; iterator += 5 /* TransferrableNodeIndex.End */) {
        ...
        if (nodeBuffer[iterator + 1 /* TransferrableNodeIndex.NodeType */] === 3 /* NodeType.TEXT_NODE */) {
          ...
          node = nodeBuffer[iterator + 4 /* TransferrableNodeIndex.Namespace */] !== 0 ? document.createElementNS(this.stringContext.get(nodeBuffer[iterator + 4 /* TransferrableNodeIndex.Namespace */]), nodeName) : document.createElement(nodeName);
...
const EventSubscriptionProcessor = (strings, nodeContext, workerContext, objectContext, config) => {
  ...
  return {
    execute(mutations, startPosition, allowedMutation) {
      ...
      const endPosition = startPosition + 4 /* EventSubscriptionMutationIndex.Events */ + addEventListenerCount * ADD_EVENT_SUBSCRIPTION_LENGTH + removeEventListenerCount * REMOVE_EVENT_SUBSCRIPTION_LENGTH;
      if (allowedExecution && allowedMutation) {
      ...
      if (allowedExecution && allowedMutation) {
        ...
        if (target) {
          ...
          workerContext.messageToWorker({
            ...
            [13 /* TransferrableKeys.target */]: [target._index_],
            [38 /* TransferrableKeys.data */]: [boundingRect.top, boundingRect.right, boundingRect.bottom, boundingRect.left, boundingRect.width, boundingRect.height]
          });
        } else {
      ...
      if (allowedExecution && allowedMutation) {
        ...
        if (target) {
          if (removeNodeCount > 0) {
            mutations.slice(startPosition + 6 /* ChildListMutationIndex.Nodes */ + appendNodeCount, startPosition + 6 /* ChildListMutationIndex.Nodes */ + appendNodeCount + removeNodeCount).forEach(removeId => {
          ...
          if (appendNodeCount > 0) {
            mutations.slice(startPosition + 6 /* ChildListMutationIndex.Nodes */, startPosition + 6 /* ChildListMutationIndex.Nodes */ + appendNodeCount).forEach(addId => {
...
const ObjectMutationProcessor = (strings, nodeContext, workerContext, objectContext, config) => {
  ...
  return {
    execute(mutations, startPosition, allowedMutation) {
      ...
      } = deserializeTransferrableObject(mutations, startPosition + 3 /* ObjectMutationIndex.SerializedTarget */, 1, strings, nodeContext, objectContext);
  ...
  return {
    execute(mutations, startPosition, allowedMutation) {
      ...
      const {
        offset: targetOffset,
        args: deserializedTarget
      } = deserializeTransferrableObject(mutations, startPosition + 4 /* ObjectCreationIndex.SerializedTarget */, 1,
      // argCount
...
function createHydrateableNode(element, minimizeString, hydrateFilter, workerContext) {
  ...
  const hydrated = {
    ...
    [2 /* TransferrableKeys.attributes */]: [].map.call(element.attributes || [], attribute => [minimizeString(attribute.namespaceURI || 'null'), minimizeString(attribute.name), minimizeString(attribute.value)])
  };
 ...
 */
const readableTransferredNode = (nodeContext, node) => node != null && nodeContext.getNode(node[0 /* TransferrableNodeIndex.Index */]) || node;
/**
 * @param node
 */
